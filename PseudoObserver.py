...
# To use this class you need two datasets. 
# The first dataset is a csv file with 3 columns and no header (observer1 est, observer2 est, and functional group name)
# The second dataset is a csv file containing data for a single observer. The file must contain columns with the following names 
# (Wetland, FuncGroup, Transect_ID, Plot_ID, Cover). If the first dataset (training dataset) is not provided then you can still use
# this class, however, you will be generating a Pseudo Observer with arbitrarilly defined default values (1 SD = 10%) for all groups in
# the real dataset. If you have at least one functional group containing pilot data then you can still use the TrainObserver function,
# which will assign the mean error rate for all functional groups you dont have data for. 

# Note there are default names for each dataset (TrainingData.csv and NP2014.csv). If your filenames match these, hit enter to bypass the 
# file name request. Also note that there are no exception handlers to define traceback errors.

class PseudoObserver:
    #set global variables
    # accessible in the instantiated variable e.g. p... using p.output
    output = {}  
    data = 0
    
    def TrainObserver():
        """
        Data table containing observer1 values (number), obsever2 values (number), and func' groups (string).\n 
        This function calculates the Mean Square Error (MSE) in cover estimates between two observers.\n
        The data is stored as a dictionary. If this function is not used, further operation using this code \n
        will result in the assignment of an arbitrary default value (+/- 10% estimation error). However, if \n
        partial data are provided here using the TrainObserver function, then the code will calculate the \n
        average of all 'real' data and assign it to the remaining functional groups.
        """
        counts = dict()
        fname = input("Enter file name of the pilot data: ")
        if len(fname) < 1: fname = "TrainingData.csv"
        with open(fname, newline = '') as f:
            reader = csv.reader(f)
            for row in reader:
                # specify the group column (functional group)
                key = row[2]
                ...
                # calculate MSE
                value =(float(row[0]) - float(row[1]))**2
                PseudoObserver.output[key] = PseudoObserver.output.get(key, 0) + value

            # divides one dictionary by another
            for entry in PseudoObserver.output:
                if entry in counts:
                    PseudoObserver.output[entry] = PseudoObserver.output.get(entry, 0) / counts[entry]
